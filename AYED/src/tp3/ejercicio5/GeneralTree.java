...
public class GeneralTree<T>{
	...
	public boolean esAncestro(T a, T b) {
		boolean yes = false;
		if (this.hasChildren() && this.getData() != null)  yes = esAncestroRecursivo(false ,false, this, a, b);
		return yes;
	}
	
	private boolean esAncestroRecursivo(boolean encontreB, boolean encontreA, GeneralTree<T> arbol, T a, T b) {
		if (arbol.getData().equals(a)) {
			encontreA = true;
		}
		if (arbol.getData().equals(b) && (encontreA)) {
			encontreB = true;
		}
		else {
			List<GeneralTree<T>> children = arbol.getChildren();
			Iterator<GeneralTree<T>> it = children.iterator();
			while (it.hasNext() && !encontreB) {
				encontreB = esAncestroRecursivo(false, encontreA, it.next(), a, b);
			}
		}
		return encontreB;
	}
	...
	public static void main(String[] args) {
	    ...
	    // Ejecutar ancho
	    System.out.println("Hay camino de a hacia b: " + root.esAncestro(7, 15));  // Debe imprimir 11
	}
}